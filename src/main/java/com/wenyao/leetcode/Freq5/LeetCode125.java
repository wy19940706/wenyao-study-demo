...
/**
 * @Description: 给定一个字符串，检测它是否是回文对称的，只考虑其中字母与数字的字符。
...
public class LeetCode125 {
    ...
    private static boolean isPalindromeTwo(String string) {
        ...
        while (left < right) {
            if (!isLetterOrNumber(string.charAt(left)) && (left < right)) {
                left++;
                continue;
            }
            if (!isLetterOrNumber(string.charAt(right)) && (left < right)) {
                right--;
                continue;
            }
            if (Character.toLowerCase(string.charAt(left)) != Character.toLowerCase(string.charAt(right)) && (left < right)) {
                return false;
            }
            left++;
            right--;
